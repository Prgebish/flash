...
- *Standalone Jump Mode*: start a flash search at any time without entering isearch.
...
=flash-isearch-mode= adds labels to your regular incremental search. It works with both =C-s= (Emacs isearch) and ~/~ / =?= (evil-ex-search).
...
| ~flash-label-position~     | ~'after~                       | Where to place labels: after, before, overlay, eol |
